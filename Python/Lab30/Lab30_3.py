...
df=pd.read_csv('000660_indi01.csv', index_col='Date')
...
price_indicator = df.loc[:,'Open':'Close'].values[0:].astype(np.float) # 가격 관련 지표 / Price Indicator
volume_indicator = df.loc[:,'Volume':'Volume'].values[0:].astype(np.float) # 거래량 관련 지표 / Volume Indicator
etc_indicator = df.loc[:,'RSI':'WilliamsR'].values[0:].astype(np.float) # 추세 또는 거래량 활용 지표 / Trend and Volume Indicator

#데이터 단위 작게 변환(normalization)
