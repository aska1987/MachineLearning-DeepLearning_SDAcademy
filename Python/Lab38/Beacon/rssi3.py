...
def loadData(path='./iBeacon_RSSI_Labeled.csv'):
    x = []
    le_col = LabelEncoder()
    le_row = LabelEncoder()
...
def main():
    ...
    model.fit(train2_X,train2_yc, epochs=100, batch_size=10,  verbose=2)
    ...
    ax1.scatter(Xs_orig, Ys_orig, color='green', marker='.')
    ax2.scatter(Xs_pred, Ys_pred, color='red', marker='.')

    plt.savefig('ibeacon.png', dpi=700)
    plt.show()
